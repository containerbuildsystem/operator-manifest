...
class TestPinCommand():
    ...
    def test_manifest_dir_empty(self, teardown, tmp_path):
        cmd = subprocess.run(['operator-manifest', 'pin', tmp_path], capture_output=True)
    ...
    def test_non_existent_manifest_dir(self, teardown):
        cmd = subprocess.run(['operator-manifest', 'pin', 'foo_dir'], capture_output=True)
    ...
    def test_digest_pinning(self, csv_filename, testing_pullspecs, teardown, tmp_path):
        copy_csv_files_to_manifest_dir([csv_filename], tmp_path)
        cmd = ['operator-manifest', 'pin', tmp_path]
        if AUTHFILE_PATH:
...
class TestExtractCommand():
    ...
    def test_manifest_dir_empty(self, teardown, tmp_path):
        cmd = subprocess.run(['operator-manifest', 'extract', tmp_path], capture_output=True)
        assert 'Missing ClusterServiceVersion in operator manifests' in str(cmd.stderr)

    def test_non_existent_manifest_dir(self, teardown):
        cmd = subprocess.run(['operator-manifest', 'extract', 'foo_dir'], capture_output=True)
        assert 'foo_dir is not a directory or does not exist' in str(cmd.stderr)
...
class TestResolveCommand():
    ...
    def test_image_file_does_not_exist(self):
        cmd = subprocess.run(['operator-manifest', 'resolve', 'nonexistentfile'], capture_output=True)
        assert "No such file or directory: 'nonexistentfile'" in str(cmd.stderr)
    ...
    def test_command(self, images_file_content, expected_data, tmp_path):
        ...
        cmd = subprocess.run(['operator-manifest', 'resolve', fp], capture_output=True)
...
class TestReplaceCommand():
    ...
    def test_non_existent_manifest_dir(self, replacements, teardown, tmp_path):
        ...
        cmd = subprocess.run(['operator-manifest', 'replace', 'foo_dir', fp], capture_output=True)
        assert 'foo_dir is not a directory or does not exist' in str(cmd.stderr)
    ...
    def test_non_existent_replacements_file(self, csv_filename, teardown, tmp_path):
        copy_csv_files_to_manifest_dir([csv_filename], tmp_path)
        cmd = subprocess.run(['operator-manifest', 'replace', tmp_path, DEFAULT_REPLACEMENTS_FILENAME], capture_output=True)
        assert f"No such file or directory: '{DEFAULT_REPLACEMENTS_FILENAME}'" in str(cmd.stderr)
    ...
    def test_command(self, csv_filename, replacements, teardown, tmp_path):
        ...
        with fp.open('w') as f:
            json.dump(replacements, f)
        subprocess.run(['operator-manifest', 'replace', tmp_path, fp], capture_output=True)
        csv_content = _get_csv_file_content(csv_filename, tmp_path)
